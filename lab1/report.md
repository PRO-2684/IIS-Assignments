...
- **单点失败风险**：广泛使用同一个密码系统可能导致单点失败 —— 一旦该系统被破解，所有依赖该系统的安全措施都会受到影响。
...
- 区块密码的操作模式是必要的，因为它们能够提供比单纯的区块加密更加丰富和安全的功能，可以确保加密的唯一性和数据的完整性。
- 在 CBC 模式中，每个明文块在加密之前都会与前一个密文块进行异或操作（第一个密文快与初始化向量 IV 异或）；加密不可以并行，加密速度相对较慢，解密可以并行；是 IND-CPA 安全的，前提是初始化向量 IV 随机；需要在末尾加 padding。
- 而 CTR 模式将明文块与加密结果进行异或操作来加密数据，选择随机生成的 Nonce (number used once) + 递增的 Counter 作为加密的输入；加密解密均可并行；是 IND-CPA 安全的，要求 Nonce 随机生成，并且不能重复使用；不需要 padding。
...
- 原因：哈希函数将任意长度的输入映射到固定长度的输出，即将无限的输入集合映射到有限的输出集合，根据抽屉原理，总会存在两个不同的输入得到相同的哈希值，即发生哈希碰撞。
- 性质：抗碰撞性，即很难找到任意两个不同的输入，它们的哈希值相同，攻击者很难通过碰撞找到另一份与原始数据哈希值相同的虚假数据。即使攻击者找到了具有相同哈希值的其他数据，由于这些数据与原始数据的语义不同，攻击者也无法通过这种方式对数据进行篡改，故冲突不是安全问题。
...
- 我们选择了第一篇论文《New Directions in Cryptography》，作者在他们的论文中开创了公钥密码学（非对称密码学）的概念，这是密码学历史上的一次革命。他们提出了非对称加密系统，其中加密和解密使用两个不同的密钥，这与之前的对称加密系统形成了鲜明对比，在对称加密系统中，加密和解密使用相同的密钥。在非对称加密系统中，有一个公开的密钥用于加密，而另一个私有的密钥用于解密。这样，即使在不安全的通信渠道上，两个没有事先共享密钥的个体也能安全地进行通信。论文的主要内容总结如下：
...
- 在区分新问题、提出新方法论和提炼独到见解方面，该论文的主要贡献包括：提出了公钥密码学的概念，这是密码学领域的一次重大突破，为安全通信提供了一种全新的方法；探索了密钥分发问题的解决方案，减少了密钥交换过程中的安全风险；引入了数字签名的概念，为电子文档和交易的认证提供了理论基础。
- 该论文也存在一些局限性，如实际应用中的计算复杂性，论文提出的理论在实际应用中可能需要高度的计算资源，特别是在当时的技术条件下；此外，尽管提出了公钥密码学的框架，但具体实现的安全性还需要进一步研究和验证。
- 现在看来，公钥密码学和数字签名的概念广泛应用于各种领域，如用于保护网络通信的安全，SSL/TLS 协议、VPN 连接等；是保护在线交易安全的核心技术，用于保障电子支付、电子合同和在线购物的安全；保护知识产权和版权，确保数字内容不被篡改或盗用；保护电子邮件的机密性和完整性，防止未经授权的人窥探或篡改邮件内容。在互联网、电子商务、金融、政府等各个领域均起着关键作用，保障了信息安全和交易安全。
